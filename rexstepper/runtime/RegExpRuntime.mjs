...
function disjunctionMatcher(m1, m2) {
    return function (state, cont) { 
        saveBranch(state, getId(state), "OR");
        var ret = m1(state, cont); 
        if (!isFailure(ret)) { 
          return ret; 
        } else { 
            saveBacktrack(state, getId(state), "OR");
...
function checkInputStringBreakPoint(state) {
    console.log("checkInputStringBreakPoint");
    console.log(state); 
    
    // Identify if we have an input string
    if (state.str.length - state.end_index < 3) return;

    var substr = state.str.substring(state.end_index, state.end_index + 3); 

    console.log("inside checkInputStringBreakPoint"); 
    console.log(substr);

    if (substr === "[!]") {
        state.end_index += 3; 
        saveInputBP(state, getId(state)); 
    }
}
...
function sequenceMatcher(m1, m2) {
    return function (state, cont) { 
        var cont_d = function (state_y) { 
          checkInputStringBreakPoint(state_y);
