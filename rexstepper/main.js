...
/* Configuration options by Miguel Gonzaga */
let verbose = {
    // Sets which tree construction algorithm is used
    // true = Miguel Gonzaga's algorithm - correct | false = Luís Almeida's algorithm - incorrect
    'newTreeAlgorithm': true,
    
    // defines if epsilon states are removed from the trace in the pre-processing stage
    // true = epsilon states are removed | false = epsilon states are kept
    'ignoreEpsilonStates': false,
}
...
function compile() {
    ...
    if(verbose.newTreeAlgorithm == true) {
        defineTreeVars();
        initializeTree(buildD3Tree(stateObject.states));
    } else {
...
function changeEditorValue(elm) {
    ...
    let regex = $(elm).children().filter('span').text(); // GNZ
...
let previousState = null; // GNZ: I could use stateObject.previousStateIndex however it also catches backtracks, which is not a visible state in the tree. This solution is cleaner
...
function updateState(index) {
    ...
    // GNZ: enable tree view knowledge of code-stepper's current state
    if(currentState.node !== undefined) {
        if(previousState != null)
            previousState.node.isCurrent = false;
        
        currentState.node.isCurrent = true;
        update(root);
...
function updateDebuggerStep(bpLeft, bpRight, stepLeft, stepRight, direction, first, last) {
    ...
    if (direction == "left") {
        if (stateObject.currentStateIndex > 0) {
            ...
            if (stateObject.states[stateObject.currentStateIndex].branch || stateObject.states[stateObject.currentStateIndex].backtrack || stateObject.states[stateObject.currentStateIndex].epsilon || stateObject.states[stateObject.currentStateIndex].textBreakPoint) {
                updateDebuggerStep(bpLeft, bpRight, stepLeft, stepRight, direction, first, last);
            }
            enableDebugButton(stepRight);
            enableDebugButton(bpRight);
...
function updateMessageBox(isError, message_elm, msg) {
    ...
    if (isError) {
        ...
        if(document.querySelector("#tree-tab").classList.contains("active")) {
            document.querySelector("#toggleExpansion").style.display = "inline-block";
        }
...
/*function numberOfBreakPoints(regex, end) {
    ...
    let condBreakPointVisual = /[!<[0-9]*,[0-9]*>]/;
    let countBreakPoints = 0;
    ...
    countBreakPoints += lookForBreakPoint.split(condBreakPointVisual).length - 1;

    return countBreakPoints;
}*/

function lengthOfBreakPoints(regex, end) {
    let locEnd = end - 1;

    let breakPointVisual = /\[!\]/g;
    let condBreakPointVisual = /\[!<[0-9]*,[0-9]*>\]/g;

    let lengthBreakPoints = 0;
    let lookForBreakPoint = regex.substring(0, locEnd); // get the part of the regex that we want (right until the current loc.end)
    console.log('lookForBreakPoint = ' + lookForBreakPoint);

    let bps = lookForBreakPoint.match(breakPointVisual);
    if(bps != null) {
        for(let i=0; i<bps.length; i++) {
            lengthBreakPoints += bps[i].length - 1;
        }
    }
    
    bps = lookForBreakPoint.match(condBreakPointVisual);
    if(bps != null) {
        for(let i=0; i<bps.length; i++) {
            lengthBreakPoints += bps[i].length - 1;
        }
    ...
    return lengthBreakPoints;
}

function isIterationInScope(min, max) {
    let iter = iterationIndex + 1; // iteration number as per end user perspective

    if(min == 0 && max == 0) return true;
    else if(min == 0 && max > 0) {
        if(iter <= max) return true;
    } else if(min > 0 && max == 0) {
        if(iter >= min) return true;
    } else if(min > 0 && max > 0 && min <= max) {
        if(iter >= min && iter <= max) return true;
    }
    return false;
}

function isBreakpointActive(bpState) {
    if(Number.isInteger(bpState.minIteration) && Number.isInteger(bpState.maxIteration)) { // is conditional break point
        if(isIterationInScope(bpState.minIteration, bpState.maxIteration))
            return true;
        else
            return false;
    } else { // is unconditional break point
        return true;
    }
}
...
function getNextBreakPoint(index, direction) {
    ...
    if (direction == "left") {
        for (let i = index; i > 0; i--) {
            if (i == index) continue;
            if (states[i].isBreakPoint) {
                if(isBreakpointActive(states[i]) == true) { // GNZ: added support for conditional break points
                    return i;
                } else continue;
            } else if(states[i].textBreakPoint) { // GNZ: added support for input string break points
                return i-1;
            }
        }
    } else if (direction == "right") {
...
function placeBreakPoints(regex) {
    /*let chars = { // https://unicode-table.com/en/sets/superscript-and-subscript-numbers/
        'bp': '\u25CF',
        '0': '\u207B',
        '1': '\u00B9',
        '2': '\u00B2',
    ...
    // replace conditional break points by dots
    let unconditionalBP = "[!]";
    var resultRegex = regex.split(unconditionalBP).join(chars['bp']);
    
    // replace unconditional break points by dots with minIteration on the top left and maxIteration on the top right
    let conditionalBP = /[!<[0-9]+,[0-9]+>]/g;
    let matches = resultRegex.match(conditionalBP); // e.g. ['[!<2,0>]' , '[!<0,3>]' , '[!<2,4>]' , '[!<0,0>]']

    if(matches != null) {
        for(let i=0; i<matches.length; i++) {
            let str = matches[i].replace('[!<', '').replace('>]', ''); // e.g. '2,0' , '0,3' , '2,4' , '0,0'
            let nums = str.split(','); // e.g. ['2', '0'] , ['0', '3'] , ['2', '4'] , ['0', '0']

            let tempStr = chars[nums[0]] + chars['bp'] + chars[nums[1]];
            resultRegex = resultRegex.replace(matches[i], tempStr);
        }
    }
    return resultRegex;*/
    ...
    // Unconditional BPs
    let ubpStr = "[!]";
    var result = regex.split(ubpStr).join(bpSymbol);

    // Conditional BPs
    let cbpStr = /\[!<[0-9]*,[0-9]*>\]/g;
    let cbps = result.match(cbpStr);
    if(cbps != null) {
        cbps.forEach((cbp) => {
            // uncomment to show break point symbol in 
            /*let cbpPattern = /\[!<(?<min>[0-9]*),(?<max>[0-9]*)>]/;
            const match = cbp.match(cbpPattern);
    
            let min = parseInt(match.groups.min) || 0;
            let max = parseInt(match.groups.max) || 0;
    
            if(isIterationInScope(min, max) == true)
                result = result.replace(cbp, bpSymbol);
            else
                result = result.replace(cbp, '');*/
            result = result.replace(cbp, bpSymbol);
        });
    }
...
function placeInputStringBreakPoints(inputStr) {
    let emptySymbol = "\u200E";
    let bpSymbol = "\u25CF";

    // Unconditional BPs
    let ubpStr = "[!]";
    var result = inputStr.split(ubpStr).join(emptySymbol + bpSymbol + emptySymbol); // keeps the break point symbol with same length as input by user
...
function styleString(state, start, end, isFail) {
    ...
    const inputString = stateObject.str;
    ...
    const stringToHighlight = placeInputStringBreakPoints(inputString);
    ...
    if (state.initial && start === 0) {
        ...
        // Fix input strings containing '<' or '>'
        let str = stringToHighlight.replace('<', '&lt;');
        str = str.replace('>', '&gt;');

        finalString = `<div class="stateTitle">String:</div> <div id="stateValue" class="${startClass}"> ${str}</div>`;
    } else if (state.initial) {
        let highlight = stringToHighlight.substring(start, stringLength);

        // Fix input strings containing '<' or '>'
        preHighlight = preHighlight.replace('<', '&lt;');
        preHighlight = preHighlight.replace('>', '&gt;');
        ...
        highlight = highlight.replace('<', '&lt;');
        highlight = highlight.replace('>', '&gt;');
        highlight_fail = highlight_fail.replace('<', '&lt;');
        highlight_fail = highlight_fail.replace('>', '&gt;');
        posHighlight = posHighlight.replace('<', '&lt;');
        posHighlight = posHighlight.replace('>', '&gt;');
...
function styleRegex(state, index, isFail) {
    ...
    const regexToHighlight = placeBreakPoints(regex);
    const regexLength = regexToHighlight.length;
    const loc = state.loc;
    const start = 0;
    ...
    lOfBreakPoints = lengthOfBreakPoints(regex, locEnd);
    ...
    // If this is NOT the end state, do not highlight the remaining substring)
    if (state.initial) {
        finalString = `<div class="stateTitle">Regex:</div> <div id="stateWrapper"><div id="stateRegex" class="${endClass}"> ${regexToHighlight}</div></div>`;
    } else if (Object.keys(loc).length === 0 && loc.constructor === Object) {
        endClass = "end";
    } else if (isFail) {
        end = loc.start - 1;
        //newEnd = end - isBP - breakPointLength * nOfBreakPoints;
        newEnd = end - lOfBreakPoints;
        start_fail = loc.start - 1;
        end_fail = loc.end - 1;
        //newStart_fail = start_fail - isBP - breakPointLength * nOfBreakPoints;
        newStart_fail = start_fail - lOfBreakPoints;
        //newEnd_fail = end_fail - isBP - breakPointLength * nOfBreakPoints;
        newEnd_fail = end_fail - lOfBreakPoints;
        posHighlight = regexToHighlight.substring(newEnd_fail, regexLength);
    } else {
        end = loc.end - 1;
        //newEnd = end - isBP - breakPointLength * nOfBreakPoints;
        newEnd = end - lOfBreakPoints;
        posHighlight = regexToHighlight.substring(newEnd, regexLength);
    }
    ...
    if (!state.initial) {
        // If this is NOT the initial state, highlight the given substring)
        let highlight_fail = "";
        let highlight = regexToHighlight.substring(start, newEnd);
        if (isFail) {
            highlight_fail = regexToHighlight.substring(newStart_fail, newEnd_fail);
        } else if (
...
$(function () {
    ...
    $(".dropdown-item").each(function () {
        ...
        item.addEventListener("click", function(event) {
            if (item === event.target || $(event.target).is('span')) { // GNZ: prevents overlapping DIV click with ANCHOR click, but keeps DIV click enabled on SPAN click
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        if(e.target.id === "tree-tab"){
            if($("#message").hasClass("clean")){
                $("#toggleExpansion").css("display", "inline-block");
            }
        }
        else {
...
function defineTreeVars() {
    ...
    console.log("NUM: " + (width + margin.right + margin.left));
    svg = d3
        .select("#treeWindow")
        .append("svg")
...
function update(source) {
    ...
    nodeEnter
        ...
        .style("fill", function (d) {
            let n = d.data.node;
            if (d.data.isCurrent) return '#1487af';
...
C ${(s.y + d.y) / 2} ${s.x},
  ...
  ${d.y} ${d.x}`;
    ...
    function click(event, d) {
        if(event.detail > 1) // GNZ: prevents visual bug on Tree-Step view when double clicking node
            return;
    ...
    // GNZ: switches view to code-step the double clicked node
    function dblclick(event, d) {
        ...
        for(let i=0; i<stateObject.states.length; i++) {
            if(stateObject.states[i] == d.data.node && (d.data.node.branch != true)) {
                stateObject.currentStateIndex = i;

                updateState(i);

                if(stateObject.currentStateIndex == 0) { // first
                    disableDebugButton(first);
                    disableDebugButton(bpLeft);
                    disableDebugButton(stepLeft);
                    ...
                    enableDebugButton(bpRight);
                    enableDebugButton(stepRight);
                } else if(stateObject.currentStateIndex == stateObject.states.length-1) { // last
                    disableDebugButton(last);
                    disableDebugButton(bpRight);
                    disableDebugButton(stepRight);
                ...
                $('.nav-tabs a[href="#' + 'debug' + '"]').tab('show');
            }
        }
    }
}
